...
// Adaptado de React Bits (reactbits.dev/animations/swarm-cursor) — MIT.
//
// No es un sistema de partículas: unas pocas gotas grandes se dibujan en un
// render target como campo escalar y un segundo pase las umbraliza (metaball),
// así que se FUSIONAN entre sí y con su propia estela. De ahí la cinta viva
// que sigue al cursor en vez de una nube de puntos.
//
// Única adaptación respecto al original: los listeners de puntero van en
// `window` en vez de en el contenedor. El original envuelve contenido y
// recibe los eventos él mismo; aquí se monta como capa fija sobre toda la
// página con pointer-events:none, que por definición no recibe ninguno.
// Las coordenadas se siguen calculando contra el rect del contenedor, así
...
const SwarmCursor = ({
  ...
  propsRef.current = {
    color, accentColor, count, size, merge, glow, opacity,
    spread, separation, speed, wander, trail, scatterOnClick, enabled,
  }
  ...
  useEffect(() => {
    ...
    // --- adaptación: los eventos se escuchan en window (ver cabecera) ---
    ...
    const frame = (now) => {
      ...
      if (!p.enabled || reduceMotion) return
